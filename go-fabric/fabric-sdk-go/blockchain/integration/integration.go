...
import (
	...
	"github.com/CunTianXing/fabric-sdk-go/pkg/config/urlutil"
...
func (setup *BaseSetupImpl) setupEventHub(client fab.Resource) error {
...
func (setup *BaseSetupImpl) GetDeployPath() string {
	pwd, _ := os.Getwd()
	return path.Join(pwd, "../../fabric-sdk-go/fixtures/testdata")
}
...
func (setup *BaseSetupImpl) getEventHub(client fab.Resource) (fab.EventHub, error) {
	...
	fmt.Printf("peerConfig: %+v\n", peerConfig)
	for _, p := range peerConfig {
		if p.URL != "" {
			// fmt.Println("============ddddd=============")
			// fmt.Printf("p.Url: %s\n", p.EventURL)
			// log.Fatalf("EventHub connect to peer (%s)", p.URL)
			fmt.Println("===========dddd==============")
			fmt.Println(p)
			fmt.Println("ddddd||||||||")
			serverHostOverride := ""
			...
			fmt.Println("serverHostOverride: ", serverHostOverride)

			fmt.Printf("p.EventURL: %s\n", p.EventURL)
